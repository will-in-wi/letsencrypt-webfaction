...
RSpec.describe LetsencryptWebfaction::CertificateInstaller do
  ...
  let(:private_key) { instance_double OpenSSL::PKey::RSA, 'private_key', to_pem: 'PRIVATE_KEY' }

  let(:certificate) { 'CERTIFICATE' }

  let(:cert_name) { 'test_auto_cert' }

  let(:cert_installer) { LetsencryptWebfaction::CertificateInstaller.new cert_name, certificate, private_key, creds }
